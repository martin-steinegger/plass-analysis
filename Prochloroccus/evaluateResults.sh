...
mmseqs search ${ASSEMBLY}.${LEN} ${REFERENCE} ${RESULT}/assembly_against_reference ${RESULT}/search1 -s 5 --max-seqs 5000 --min-ungapped-score 100  -a --min-seq-id 0.89 > ${RESULT}/search1.log
...
mmseqs search $REFERENCENR ${ASSEMBLY}.${LEN} ${RESULT}/reference_against_assembly ${RESULT}/search2 --max-seqs 500000  -a --min-seq-id 0.89 > ${RESULT}/search2.log
for i in $(seq 90 99| awk '{print $1/100}'); do
